...
export async function saveInstallToken(tokens) {
  // ── 진단 로그 (토큰 길이/필드 확인용) ──
  console.log('[saveInstallToken] cafe24 raw response keys:', Object.keys(tokens || {}));
  console.log('[saveInstallToken] token lengths:', {
    access_token_len: tokens?.access_token ? String(tokens.access_token).length : 0,
    refresh_token_len: tokens?.refresh_token ? String(tokens.refresh_token).length : 0,
    expires_at: tokens?.expires_at,
    refresh_token_expires_at: tokens?.refresh_token_expires_at,
    issued_at: tokens?.issued_at,
  });
  console.log('[saveInstallToken] full payload:', JSON.stringify(tokens));
