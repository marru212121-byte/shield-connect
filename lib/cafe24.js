...
// 3) 카페24 API로 주문 조회 (존재 확인만, 결제 상태는 카페24가 이미 검증)
...
// 결제 검증: 카페24가 주문을 반환하면 이미 검증된 것
// (선불금결제완료, 배송준비중 등 정상 상태면 모두 인정)
// 명시적 "취소/환불/반품"만 거부
function isCanceledOrRefunded(order) {
  ...
  // 명시적 취소/환불 상태만 거부
  if (['C10', 'C20', 'C30', 'C40'].includes(os)) return true;  // 주문 취소
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 3) 취소/환불만 거부 (나머지는 카페24가 이미 검증한 정상 주문)
